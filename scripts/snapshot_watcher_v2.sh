#!/usr/bin/env bash
# Snapshot-soup watcher v2 (2026-09-03: double memory check): as each decay run of plan v3 finishes
...
while :; do
  ...
  while IFS=: read -r st suffix cfg; do
    ...
    if [ -e "$ORC/$st.done" ]; then
      ...
      # two low readings 180 s apart: a training container ramps to 13 GB
      # within minutes of its start, and a single reading raced that ramp
      # (2026-09-03: an eval started in the ramp and one graph OOMed)
      while :; do
        m1="$(nvidia-smi --query-gpu=memory.used --format=csv,noheader,nounits | head -1)"; sleep 180
        m2="$(nvidia-smi --query-gpu=memory.used --format=csv,noheader,nounits | head -1)"
        [ "$m1" -le 8000 ] && [ "$m2" -le 8000 ] && break
      done

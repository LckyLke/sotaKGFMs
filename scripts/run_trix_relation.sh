...
#
# TRIX has no run_many.py. src/run_relation.py takes exactly one dataset per
...
CKPT="$WORKDIR/relation_prediction.pth"
DATA_ROOT="$ROOT/data/roots/trix"
RANKS="${TRIX_RANKS:-$ROOT/ranks-relation/trix}"
OUT="$ROOT/output/trix/${TRIX_SHARD:-all}"

case "$GROUP" in
  transductive) CONFIG="$WORKDIR/config/run_relation_transductive.yaml" ;;
  ind_e|ind_er) CONFIG="$WORKDIR/config/run_relation_inductive.yaml" ;;
...
if [ -f "$PROV" ]; then
  ...
  if [ "$had" != "$DEVICE" ] && [ -z "${TRIX_REDO:-}" ]; then
    ...
    echo "Clear it first:  rm -rf $RANKS $ROOT/ranks/.claims-trix-relation" >&2
    exit 3
  fi
fi
...
CLAIMS="$ROOT/ranks/.claims-trix-relation"
mkdir -p "$CLAIMS"
cd "$WORKDIR"
...
for d in ${DATASETS//,/ }; do
  ...
  if TRIX_RANK_DUMP_DIR="$RANKS" $PY "$WORKDIR/src/run_relation.py" \
      -c "$CONFIG" \
      --dataset "$DS" \
      --version "$VERSION" \
      --gpus "$GPUS" \

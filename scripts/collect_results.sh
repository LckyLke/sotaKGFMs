#!/usr/bin/env bash
# Copy ULTRA's raw ultra_results_*.csv out of the work tree into results/.
#
# run_many.py writes them next to itself inside the tree, which
# prepare_ultra_workdir.sh wipes on rebuild. They are a deliverable and the
...
echo "collected $n csv(s) into results/"
ls -1 "$ROOT/results" 2>/dev/null | sed 's/^/  /'

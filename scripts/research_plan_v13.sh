...
dev_eval() {
  ...
  ( cd "$INC" && $ALLOC scripts/docker_run.sh incite bash -c "
      ...
      python /kgfm-src/diagnostics/dev_eval.py --config $cfg --ckpt $ckpt \
        --val_samples 500 --batch_size 16 --out /kgfm-src/results/incite/dev/$name.json" ) >> "$LOG" 2>&1
...
say "=== research plan v13 start (pid $$) ==="

# ---- takeover from v12 at its stage boundary ------------------------------
...
PRED='^bash scripts/research_plan_v12\.sh'
...
# The paper recipe is MX1's (the mix in the decay phase only). SC1 joins it
# if BOTH hold: its dev-suite mean exceeds MX1's by at least 0.003, and the
# paired 41-graph bootstrap against MX1 has an interval above zero on at
# least one group while the other group's point estimate is above -0.002.
# No test-set argmax anywhere else.
recipe_decision() {
  python3 - "$INC" "$ROOT" <<'PY'
...
def dev(name):
    ...
    return json.load(open(p))["mean"] if os.path.exists(p) else None
sc1, mx1 = dev("SC1"), dev("MX1")
join = False
reason = "no dev numbers"
if sc1 is not None and mx1 is not None:
    reason = "dev SC1 %.4f vs MX1 %.4f" % (sc1, mx1)
    if sc1 - mx1 >= 0.003:
        out = subprocess.run([sys.executable, os.path.join(root, "scripts/paired_bootstrap.py"),
                              os.path.join(inc, "ranks/incite-4g-synth30-scenario-last"),
                              os.path.join(inc, "ranks/incite-4g-synth30-last")],
        ...
        if len(cells) == 2:
            ...
            excl = (e[1] > 0) or (er[1] > 0)
            other_ok = (e[0] >= -0.002) and (er[0] >= -0.002)
            join = excl and other_ok
            reason += "; test ind_e %+.4f [%+.4f, %+.4f], ind_er %+.4f [%+.4f, %+.4f]" % (e + er)
        else:
            reason += "; no paired bootstrap"
print(("mx1sc" if join else "mx1") + "|" + reason)
PY
}
...
if [ "$RECIPE" = mx1sc ]; then
  RCFG=/kgfm-src/configs/incite_recipe_mx1_scenario.yaml

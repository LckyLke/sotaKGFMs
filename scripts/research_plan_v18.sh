#!/usr/bin/env bash
# Research plan v18 (2026-09-04, 15:05, machine clock): v17 plus stage SW1, the
# combination probe's two one-pass checkpoints (results/incite/COMBINATION_PROBE.md,
# a CPU study of 4 Sep): the round-wise swap of L1's rounds 0-2 and heads with
# MX15's rounds 3-5 (+0.0024 over L1 on held-out carved splits) and the 0.5
# soup of L1 and MX15. Evaluation only, no training: the dev suite for both,
# the 41 graphs for one within 0.002 of MX15's dev number. Placed after
# MXS9, before MX2a. Not recipe candidates (post-hoc combinations). Takes
# over from v17 at the stage boundary. Everything else is v17 verbatim.
#
# Research plan v17 (2026-09-04, 08:47, machine clock): v16 plus stage PG3, the
# gate that can close (Luke's call, 4 Sep, after PG2 turned out inert by
# construction: bias 6, one-sided loss). PG3 = MX1's recipe plus the gate
# at bias 2 with the two-sided proof loss (configs/incite_phase1_4g_synth30_gate3.yaml),
# warm start with MX1's synthetic stream, a recipe candidate placed after
# RR2 and before the recipe decision (mx1gate3: incite_recipe_mx1_gate3.yaml,
# R0 control incite_phase1_4g_gate3.yaml). Takes over from v16 at the
# stage boundary. Everything else is v16 verbatim (its header follows).
#
...
say "=== research plan v18 start (pid $$) ==="

# ---- takeover from v12/v13/v14/v15/v16/v17 at the stage boundary ----------
...
PRED='^bash scripts/research_plan_v1[234567]\.sh'
...
# ---- SW1: the combination probe's one-pass checkpoints (evaluation only) ----
# results/incite/COMBINATION_PROBE.md (4 Sep, CPU): the round-wise swap (L1's
# rounds 0-2 and heads, MX15's entity and relation steps of rounds 3-5) and
# the 0.5 soup of L1 and MX15, both under configs/incite_phase1.yaml. The dev
# suite for both; the 41 graphs for one within 0.002 of MX15's dev number.
SW1_DIR=/kgfm-src/output/incite-combo/soups
if ! skip SW1; then
  ok=1
  dev_eval $SW1_DIR/part_late100_mx15.pth /kgfm-src/configs/incite_phase1.yaml SW1swap || ok=0
  dev_eval $SW1_DIR/soup_L1_MX15_a50.pth /kgfm-src/configs/incite_phase1.yaml SW1soup || ok=0
  python3 - "$INC" <<'PY' > "$ORC/SW1.pick"
import json, os, sys
inc = sys.argv[1]
def mean(name):
    ...
    if not os.path.exists(p):
        return None
    j = json.load(open(p))
    return j["mean"] if j.get("protocol") == "stratified_v2" else None
ref = mean("MX15")
for tag, name in (("swap", "SW1swap"), ("soup", "SW1soup")):
    m = mean(name)
    if ref is not None and m is not None and m >= ref - 0.002:
        print(tag)
PY
  say "SW1: 41-graph evaluation for: $(tr '\n' ' ' < "$ORC/SW1.pick")"
  for pick in $(cat "$ORC/SW1.pick"); do
    case "$pick" in
      swap) ck=part_late100_mx15; dump=incite-swap-l1mx15-last ;;
      soup) ck=soup_L1_MX15_a50; dump=incite-soup-l1mx15-last ;;
    esac
    incite_eval $SW1_DIR/$ck.pth /kgfm-src/configs/incite_phase1.yaml $dump || ok=0
  done
  [ "$ok" -eq 1 ] && done_mark SW1 || fail_mark SW1
fi
...
# ---- PG3: MX1 plus the gate that can close (vs MX1), a recipe candidate ----
if ! skip PG3; then
  if trained 4g-synth30-gate3 || {
       seed_pretrain_dir PG3 &&
       PLAN_INIT_FROM=/kgfm-src/output/incite-pretrain-4g/incite_last.pth \
       incite_pretrain PG3 /kgfm-src/configs/incite_phase1_4g_synth30_gate3.yaml 4g-synth30-gate3 10000 \
         INCITE_TRAIN_GRAPHS=FB15k237,WN18RR,CoDExMedium,NELL995 \
         INCITE_TRAIN_EXTRA_ARGS="$UDECAY"; }; then
    dev_eval /kgfm-src/output/incite-pretrain-4g-synth30-gate3/incite_last.pth /kgfm-src/configs/incite_phase1_4g_synth30_gate3.yaml PG3 || say "PG3: dev eval failed (continuing)"
    if incite_eval /kgfm-src/output/incite-pretrain-4g-synth30-gate3/incite_last.pth \
         /kgfm-src/configs/incite_phase1_4g_synth30_gate3.yaml incite-4g-synth30-gate3-last; then done_mark PG3; else fail_mark PG3; fi
  else
    fail_mark PG3
  fi
fi
...
# ONE modification: among SC1, MX15, MX45, MXS9, RR2 and PG3, a candidate is
...
cands = {"mx1sc": ("SC1", "incite-4g-synth30-scenario-last"),
         ...
         "mx1gate3": ("PG3", "incite-4g-synth30-gate3-last")}
...
case "$RECIPE" in
  ...
  mx1gate3) RCFG=/kgfm-src/configs/incite_recipe_mx1_gate3.yaml
            R0CFG=/kgfm-src/configs/incite_phase1_4g_gate3.yaml ;;

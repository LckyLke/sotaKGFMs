...
say "=== research plan v10 start (pid $$) ==="
...
PRED='^bash scripts/research_plan_v[89].sh'
if pgrep -f "$PRED" > /dev/null; then
  ...
  if [ -z "$cur" ]; then
    ...
    n0="$(ls "$ORC"/*.done "$ORC"/*.failed 2>/dev/null | wc -l)"
    until [ -f "$INC/output/incite-pretrain/STAGE" ] || [ "$(ls "$ORC"/*.done "$ORC"/*.failed 2>/dev/null | wc -l)" -gt "$n0" ] || ! pgrep -f "$PRED" > /dev/null; do
      sleep 15
    done
    cur="$(cat "$INC/output/incite-pretrain/STAGE" 2>/dev/null || true)"
  fi

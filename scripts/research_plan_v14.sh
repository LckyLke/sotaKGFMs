...
say "=== research plan v14 start (pid $$) ==="

# ---- takeover from v12/v13 at the stage boundary ---------------------------
...
PRED='^bash scripts/research_plan_v1[23]\.sh'
...
for tag, (stage, dump) in cands.items():
    d = dev(stage)
    if d is None or mx1 is None:
        notes.append("%s: no dev number" % stage); continue
    gain = d - mx1

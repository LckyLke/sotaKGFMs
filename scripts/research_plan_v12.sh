...
say "=== research plan v12 start (pid $$) ==="

# ---- takeover from v11 NOW: MX2 lost and v11 had just started PG1 on
# MX2's recipe. Stop v11 and PG1's container, drop PG1's minutes-old run
# directory if it holds no checkpoint yet, and continue with the levers
# rebased on MX1's recipe.
PRED='^bash scripts/research_plan_v11\.sh'
if pgrep -f "$PRED" > /dev/null; then
  pkill -f "$PRED" && say "v11 stopped"
  sleep 2
  for c in $(docker ps --format '{{.ID}} {{.Image}}' | grep " kgfm/incite:" | cut -d' ' -f1); do
    docker stop "$c" > /dev/null && say "stopped container $c (kgfm/incite, v11's PG1 on the MX2 recipe)"
  done
  d="$INC/output/incite-pretrain"
  if [ "$(cat "$d/STAGE" 2>/dev/null)" = PG1 ] && ! ls "$d"/*.pth >/dev/null 2>&1; then
    rm -rf "$d" && say "dropped PG1's run directory (no checkpoint yet)"
  fi
fi
...
# ---- RR2: MX2a plus rule recovery from the relation states (vs MX2a) ----
if ! skip RR2; then
...
# ---- MXS2: MX1 with the query draw at the benchmark's unseen-answer share (vs MX1) ----
if ! skip MXS2; then
  if trained 4g-synth30-s37 || {
       seed_pretrain_dir MXS2 "$INC/output/incite-pretrain-4g/incite_last.pth" &&
       incite_pretrain MXS2 /kgfm-src/configs/incite_phase1_4g_synth30_share37.yaml 4g-synth30-s37 30000 \
         INCITE_TRAIN_GRAPHS=FB15k237,WN18RR,CoDExMedium,NELL995 \
         INCITE_TRAIN_EXTRA_ARGS="$DECAY"; }; then
    if incite_eval /kgfm-src/output/incite-pretrain-4g-synth30-s37/incite_last.pth \
         /kgfm-src/configs/incite_phase1.yaml incite-4g-synth30-s37-last; then done_mark MXS2; else fail_mark MXS2; fi
  else
    fail_mark MXS2
  fi
fi

# ---- MX2b: MX1 plus 64 uniform certified negatives and 4 positives, bisection step two (vs MX1) ----
if ! skip MX2b; then
  if trained 4g-synth30-neg64 || {
       seed_pretrain_dir MX2b "$INC/output/incite-pretrain-4g/incite_last.pth" &&
       incite_pretrain MX2b /kgfm-src/configs/incite_phase1_4g_synth30_neg64.yaml 4g-synth30-neg64 30000 \
         INCITE_TRAIN_GRAPHS=FB15k237,WN18RR,CoDExMedium,NELL995 \
         INCITE_TRAIN_EXTRA_ARGS="$DECAY"; }; then
    if incite_eval /kgfm-src/output/incite-pretrain-4g-synth30-neg64/incite_last.pth \
         /kgfm-src/configs/incite_phase1.yaml incite-4g-synth30-neg64-last; then done_mark MX2b; else fail_mark MX2b; fi
  else
    fail_mark MX2b
  fi
fi
...
cands = {"L1": "incite-4g-decay-last", "G1": "incite-4g-unary-last",
         ...
         "PG2": "incite-4g-synth30-gate-last", "MX2a": "incite-4g-synth30-iso-last",
         "RR2": "incite-4g-synth30-iso-rules-last", "MXS2": "incite-4g-synth30-s37-last",
         "MX2b": "incite-4g-synth30-neg64-last"}
...
case "$WIN" in
  ...
  PG2)  WCFG=/kgfm-src/configs/incite_phase1_4g_synth30_gate.yaml ;;
  MX2a) WCFG=/kgfm-src/configs/incite_phase1_4g_synth30_iso.yaml ;;
  RR2)  WCFG=/kgfm-src/configs/incite_phase1_4g_synth30_iso_rules.yaml ;;
  MXS2) WCFG=/kgfm-src/configs/incite_phase1_4g_synth30_share37.yaml ;;
  MX2b) WCFG=/kgfm-src/configs/incite_phase1_4g_synth30_neg64.yaml ;;
  *)    WCFG="" ;;
esac

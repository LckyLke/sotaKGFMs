...
if pgrep -f '^bash scripts/research_plan_v4.sh' > /dev/null; then
  echo "research_plan_v4.sh already running"
else
  nohup scripts/research_plan_v4.sh >> output/research-plan/nohup.log 2>&1 & disown
  echo "research_plan_v4.sh launched"
fi

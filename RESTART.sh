...
# scripts/research_plan_v17.sh's header for why. Do not relaunch them.
#
...
if pgrep -f '^bash scripts/research_plan_v17.sh' > /dev/null; then
  echo "research_plan_v17.sh already running"
else
  nohup scripts/research_plan_v17.sh >> output/research-plan/nohup.log 2>&1 & disown
  echo "research_plan_v17.sh launched"
fi
